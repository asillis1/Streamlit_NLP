...
if file is not None:
    ...
    type = st.radio("Which review types do you want to include?",
    ...
    # State column and filtering logic here (if needed)

    # Process file based on config options using functions.py
    data = functions.state(data)  # Assuming this creates a "state" column
    data = functions.process_files(data, type)
    ...
    if metric == "Sentiment Distribution":
        ...
        for category, plot_html in sentiment_trend_plots.items():
            ...
            st.write(plot_html)  # Assuming plot_html contains HTML content
